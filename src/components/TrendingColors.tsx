  ...
  return (
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl lg:rounded-2xl p-4 lg:p-6 border border-gray-700">
      <div className="flex items-center gap-3 mb-4 lg:mb-6">
        ...
        <h3 className="text-lg lg:text-xl font-semibold text-white">Couleurs Tendance</h3>
        <div className="ml-auto bg-green-500/20 text-green-400 px-2 lg:px-3 py-1 rounded-full text-xs lg:text-sm font-medium">
          {currentSeason} 2024
        </div>
      </div>
      ...
      <div className="mb-6 lg:mb-8">
        <div className="flex items-center gap-2 mb-3 lg:mb-4">
          ...
          <h4 className="text-base lg:text-lg font-semibold text-white">Collection {currentSeason}</h4>
        </div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3">
          {currentPalette.map((colorItem) => (
            <button
              ...
              className={`group relative p-3 lg:p-4 rounded-lg lg:rounded-xl border-2 transition-all hover:scale-105 ${
            ...
            >
              <div className="absolute inset-0 bg-black/40 rounded-lg lg:rounded-xl"></div>
              <div className="relative z-10 text-center">
                <div className="text-white font-semibold text-xs lg:text-sm mb-1">
                  {colorItem.name}
                </div>
                <div className="bg-green-500/80 text-white text-xs px-1.5 lg:px-2 py-0.5 lg:py-1 rounded-full">
                  {colorItem.trend}
                </div>
              </div>
            </button>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3 lg:mb-4">
          ...
          <h4 className="text-base lg:text-lg font-semibold text-white">Plus Populaires</h4>
        </div>
        <div className="grid grid-cols-1 gap-1.5 lg:gap-2">
          {popularColors.map((colorItem) => (
            <button
              key={colorItem.name}
              onClick={() => onColorSelect(colorItem.color)}
              className={`w-full flex items-center gap-2 lg:gap-3 p-2 lg:p-3 rounded-lg border transition-all hover:scale-[1.02] ${
                currentColor === colorItem.color
            ...
            >
              <div
                className="w-5 h-5 lg:w-6 lg:h-6 rounded-full border border-gray-500 flex-shrink-0"
                style={{ backgroundColor: colorItem.color }}
              />
              <div className="flex-1 text-left">
                <div className="text-white font-medium text-xs lg:text-sm">{colorItem.name}</div>
                <div className="text-gray-400 text-xs hidden lg:block">{colorItem.category}</div>
              </div>
              <div className="text-right">
                <div className="text-green-400 font-semibold text-xs lg:text-sm">
